...
st.title("Concise Dynamic Prompt Builder")
...
with st.form("prompt_form"):
    ...
    for i, h in enumerate(st.session_state.headers):
        cols = st.columns([2, 2, 3])  # compact row layout
        h["header"] = cols[0].text_input("Header Name", value=h["header"], key=f"header_{i}")
        ...
        h["desc"] = cols[2].text_input("Short Description", value=h["desc"], key=f"desc_{i}")

    st.form_submit_button("➕ Add Header", on_click=add_header)

    st.subheader("Rules")
    for i, r in enumerate(st.session_state.rules):
        st.text_input(f"Rule {i+1}", value=r, key=f"rule_{i}")
